...
public class Explodable : MonoBehaviour
{
    ...
    public void Start(){
        extraPoints = (int)(this.gameObject.transform.localScale.x * this.gameObject.transform.localScale.y * extraPointsScale);
        Debug.Log("extraPoints");
        this.fragmentInEditor();
        //EditorUtility.SetDirty(myTthisarget);
    }
    /// <summary>
    ...
    private void generateFragments()
    {
        ...
        switch (shatterType)
        {
            case ShatterType.Triangle:
                fragments = SpriteExploder.GenerateTriangularPieces(gameObject, extraPoints, subshatterSteps,mat);
                break;
        ...
        foreach (GameObject p in fragments)
        {
            if (p != null)
            {
                p.layer = LayerMask.NameToLayer("Ice");
                p.GetComponent<Renderer>().sortingLayerName = "Ice";
                ...
                p.GetComponent<PolygonCollider2D>().isTrigger = true;
                p.SetActive(true);
